...
SCRIPT_DIR="$1"
TEST_PORT=$2
EXPECTED_RESPONSE_FILE="${SCRIPT_DIR}/../test_static_root/test_sudo-rm-rf.txt"
ACTUAL_RESPONSE_FILE="${SCRIPT_DIR}/actual_static_txt_response.txt"

# Send a request to the web server to get the static file and save the output to ACTUAL_RESPONSE_FILE
curl -k -o "$ACTUAL_RESPONSE_FILE" -s -S https://localhost:$TEST_PORT/static/test_sudo-rm-rf.txt -H "User-Agent:"
